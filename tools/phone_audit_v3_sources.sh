...
OUT="/storage/emulated/0/Download/model0001-v3-source-audit.json"
SELF="$(cd "$(dirname "$0")" && pwd)"
PY="/root/mobilellm-ref/.venv/bin/python"

if [ -x "$PY" ]; then
  "$PY" "$SELF/audit_model0001_v3_sources.py" --project "$PROJECT" --output "$OUT"
